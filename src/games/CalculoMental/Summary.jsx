...
const ANDROID_GAME_CONFIG = {
    ...
    configPath: 'android/app/src/main/assets/public/config/calculo-config.json',
    buildConfigData: ({ gameConfig, gameDetails, selectedPlatforms }) => {
        ...
        return {
            nivel: nivelMap[gameConfig?.level] || 'basico',
            ejercicios: gameConfig?.exerciseCount || 1,
...
const Summary = ({ config = {}, onBack }) => {
    ...
    const MOCK_DATA = {
        selectedAreas: ['math'],
        selectedSkills: ['Resolución de problemas', 'Cálculo rápido'],
        gameDetails: {
    ...
    const generarAndroidZipConConfig = async () => {
        try {
            ...
            const nombreArchivo = `android-calculomental-${configData.nivel}.zip`
                .normalize('NFD')
                .replace(/[\u0300-\u036f]/g, '')
                .replace(/\s+/g, '-')
                .toLowerCase();
    ...
    return (
        <div className="summary-screen">
            ...
            <div className="summary-details" style={{ maxWidth: '800px', margin: '0 auto' }}>
                <div className="info-grid">
                    <div className="info-card">
                        ...
                        <div className="info-card-value">{gameDetails.gameName || 'Cálculo Mental'}</div>
                    </div>
                    ...
                    <div className="info-card full-width">
                        ...
                        <div className="info-card-value" style={{ fontSize: '1rem', lineHeight: '1.5' }}>
                            {gameDetails.description || 'Juego de cálculo mental para mejorar habilidades matemáticas.'}
                        </div>
                    </div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '2rem', rowGap: '1rem', alignItems: 'center' }}>
                    <div className="summary-row">
                        ...
                        <strong style={{ fontSize: '1.1rem', color: '#0077b6' }}>
                            {(gameConfig.level || 'basico').charAt(0).toUpperCase() + (gameConfig.level || 'basico').slice(1)}
                        </strong>
                    </div>

                    <div className="summary-row">
                        <span style={{ display: 'flex', gap: '8px', alignItems: 'center', color: '#64748b' }}>
                            <List size={18} /> Número de ejercicios:
                        </span>
                        <strong style={{ fontSize: '1.1rem', color: '#0077b6' }}>
                            {gameConfig.exerciseCount || 1}
                        </strong>
                    </div>
                </div>
            ...
            {/* Sección de Descarga Android */}
            <div className="download-section">
                <div style={{ width: '100%', maxWidth: '600px', textAlign: 'center' }}>
                    <h3 style={{ color: '#0077b6', marginBottom: '0.5rem' }}>Descargar Paquete Android</h3>
                    <p style={{ color: '#64748b', marginBottom: '1.5rem' }}>
                        Genera el proyecto React Native listo para compilar en Android Studio.
                    </p>
                ...
                <p style={{ fontSize: '0.85rem', color: '#64748b', maxWidth: '600px', margin: '0.5rem 0 0 0', textAlign: 'center' }}>
                    Proyecto React Native listo para compilar en Android Studio y generar el APK.
                </p>
            </div>
        </div>
    );
